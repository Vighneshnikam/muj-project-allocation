...
function validateEmail($email) {
    $email = trim($email);
    $basic_valid = filter_var($email, FILTER_VALIDATE_EMAIL);
    $domain_valid = preg_match('/@muj\.manipal\.edu$/', $email);
    
    // Add debug output
    error_log("Validating email: $email - Basic validation: " . ($basic_valid ? "Pass" : "Fail") . 
              ", Domain validation: " . ($domain_valid ? "Pass" : "Fail"));
    
    return $basic_valid && $domain_valid;
}
